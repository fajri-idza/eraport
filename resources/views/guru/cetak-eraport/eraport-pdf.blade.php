...
<table class="no-border" style="margin-left: 15px;margin-right:15px;line-height:0.4;font-size:12px">
    ...
    <tr>
        <td><strong>Sekolah</strong></td><td>: Genjea</td>
        <td style="width: 6%"><strong>Semester</strong></td><td>: {{ $eraport->semester }}</td>
    </tr>
    <tr><td></td><td><td style="width: 20%"><strong>Tahun Pelajaran</strong></td><td>: {{ $eraport->tahun }}</td></td></tr>
</table>

<table class="table table-bordered" style="font-size: 10px; padding: 2px">
    <thead>
        <tr>
        ...
        @foreach($nilai as $index => $n)
            ...
            @if($rowsUsed < $maxRows)
                <tr>
                    ...
                    <td>{{ $n->muatan->capaian_kompetensi }}</td>
                </tr>
                @php $rowsUsed++; @endphp
            @endif
        @endforeach
